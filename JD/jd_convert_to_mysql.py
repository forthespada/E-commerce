...
def main():
	...
	df = pd.read_csv(r'F:\jd_comment.csv')
	...
	for i in range(num):
		...
		cursor = db.cursor()
		product_sql = "INSERT INTO jd_product(product_id,product_name,product_url,product_price,product_description,product_categories,time) VALUES(%s,%s,%s,%s,%s,%s,%s)"
		...
		if item['comments'] is not np.nan:
			...
			for comment in comments:
				# print("comment['comment_user_id']",comment['comment_user_id'])
				# print("comment['comment_user_id']['$numberLong']",comment['comment_user_id']['$numberLong'])
				try:
					review_id = comment['comment_user_id']['$numberLong'] if comment['comment_user_id']['$numberLong'] is not np.nan else None
				except:
				...
				# review_content = review_content.replace('\\','').replace(r'\\','')
				...
				if '用户未及时' in review_content or '未填写评价' in review_content:
					continue
				else:
					review_sql = "INSERT INTO jd_review(product_id,review_id,reviewer_nickname,review_content,review_rating,review_helpful,review_time,time) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"
					current_time = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
					cursor.execute(review_sql,(product_id,review_id,reviewer_name,str(review_content),review_rating,review_helpful,review_time,current_time))
					db.commit()  #提交数据
		else:
			# print(i)
			print('此商品评论为空')
