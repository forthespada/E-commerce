...
def main():
	...
	df = pd.read_csv(r'F:\guomei_comment.csv')
	...
	for i in range(num):
		print(i)
		item = df.ix[i,:]
		product_url = 'https://item.gome.com.cn/' + item['product_id']+ '.html'
		...
		cursor = db.cursor()
		product_sql = "INSERT INTO guomei_product(product_id,product_name,product_url,product_price,product_description,product_categories,time) VALUES(%s,%s,%s,%s,%s,%s,%s)"
		...
		if item['comments'] is not np.nan:
			...
			review_sql = "INSERT INTO guomei_review(product_id,review_id,reviewer_nickname,review_content,review_rating,review_helpful,review_time,time) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)"
			for comment in comments:
				...
				if '用户未及时' in review_content or '没有填写' in review_content:
					continue
				else:
					current_time = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
